      ...
      <div v-else class="sf-table-wrap"><table class="sf-table">
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="c in filteredCustomers" :key="c.id" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3 text-gray-500 max-w-xs truncate">{{ c.notes || '—' }}</td>
            <td class="px-4 py-3">
              <div class="flex items-center justify-center gap-3">
                <a
                  v-if="whatsappLink(c.phone)"
                  :href="whatsappLink(c.phone, defaultMessage(c))!"
                  target="_blank"
                ...
                <a
                  v-if="mailtoLink(c.email)"
                  :href="mailtoLink(c.email, 'SmartFlow')!"
                ...
                <span v-if="!whatsappLink(c.phone) && !mailtoLink(c.email)" class="text-gray-300 text-xs">—</span>
              </div>
            </td>
            <td class="px-4 py-3 text-center">
              <div class="flex items-center justify-center gap-2">
                <button @click="openEdit(c)" class="text-blue-500 hover:text-blue-700 transition-colors" title="تعديل">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                  </svg>
                </button>
                <button @click="confirmDelete(c)" class="text-red-500 hover:text-red-700 transition-colors" title="حذف">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div v-if="showModal" class="sf-modal-backdrop" dir="rtl">
        <div class="sf-modal-panel max-w-md">
          <div class="flex justify-between items-center mb-6">
            <h3 class="text-xl font-bold text-gray-900">{{ editTarget ? 'تعديل عميل' : 'عميل جديد' }}</h3>
    ...
    <Teleport to="body">
      <div v-if="deleteTarget" class="sf-modal-backdrop" dir="rtl">
        <div class="sf-modal-panel max-w-sm text-center">
          ...
          <p class="text-lg font-bold text-gray-900 mb-2">حذف العميل "{{ deleteTarget.name }}"؟</p>
          ...
          <div class="flex gap-3">
            ...
            <button @click="deleteCustomer" :disabled="deleteLoading" class="flex-1 bg-red-600 hover:bg-red-700 disabled:opacity-60 text-white py-2.5 rounded-lg text-sm font-medium transition-colors">حذف</button>
          </div>
        </div>
      </div>
    </Teleport>
...
const defaultMessage = (c: Customer) => `مرحباً ${c.name}، معك فريق SmartFlow.`
...
const submitForm = async () => {
  ...
  try {
    if (editTarget.value) {
      const res = await api.patch(`/admin/customers/${editTarget.value.id}`, form)
      const idx = customers.value.findIndex((c) => c.id === editTarget.value!.id)
...
const deleteCustomer = async () => {
  ...
  try {
    await api.delete(`/admin/customers/${deleteTarget.value.id}`)
