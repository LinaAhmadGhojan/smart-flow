  ...
  <div dir="rtl" class="flex flex-col h-[calc(100vh-4rem)] min-h-[480px] -m-4 sm:-m-6">
    <div class="flex items-center justify-between gap-3 px-4 sm:px-6 py-3 bg-white border-b border-gray-200 shrink-0 flex-wrap">
      <div class="flex items-center gap-3 min-w-0">
        ...
        <div class="min-w-0">
          <h1 class="text-lg font-bold text-gray-900 truncate">وصل استلام مالي</h1>
          <p class="text-xs text-gray-500 truncate">{{ receiptLabel }}</p>
        </div>
      </div>
    ...
    <div v-if="loading" class="flex-1 flex items-center justify-center bg-gray-100">
      <div class="text-center">
        ...
        <p class="text-sm text-gray-500 mt-3">جاري تحميل الوصل...</p>
      </div>
    </div>
...
const receiptLabel = computed(() => `RCP-${projectId.value}-${paymentId.value.padStart(3, '0')}`)
...
const loadHtml = async () => {
  ...
  try {
    htmlContent.value = await fetchAdminHtml(htmlPath.value)
  } catch (e: any) {
    error.value = e.message || e.response?.data?.message || 'تعذر تحميل الوصل'
  } finally {
    loading.value = false
  }
}
